...
def populate_train_list(orig_images_path, hazy_images_path):
	...
	image_list_haze = glob.glob(hazy_images_path + "*.png")   # png  png  hjy 4-26png

	tmp_dict = {}

	for image in image_list_haze:
		image = image.split("\\")[-1]  # png
	#	print(image)
		#key = image.split("_")[0] + "_" + image.split("_")[1] + '.png'  # 4-26
		key = image
			  # + "_"+  image.split("_")[2] + "_"+  image.split("_")[3] + "_" + image.split("_")[4]  # name length  hjy
		# print(key)
	#	print(key)  # 0
...
class dehazing_loader(data.Dataset):
	...
	def __getitem__(self, index):
		...
		data_orig = data_orig.resize((256,512), Image.ANTIALIAS)  # 480 640
		data_hazy = data_hazy.resize((256,512), Image.ANTIALIAS)
